...
export default {
  Query: {
    async managerConfig() {
      ...
      try {
//         const raw = await db.getRaw('js_config', `SELECT * FROM js_config INNER JOIN js_config_balances ON js_config.id = js_config_balances.fk_config_id WHERE js_config.id IN
// (SELECT DISTINCT(js_config.id) FROM js_config INNER JOIN js_contract_block_piv as cb ON js_config.fk_cb_id = cb.id INNER JOIN blocks as b ON cb.fk_block_id = b.id INNER JOIN js_contracts as c ON c.id = cb.fk_contract_id INNER JOIN js_chain_network as cn ON cn.id = c.fk_chain_network_id INNER JOIN chain_network ON chain_network.id = b.fk_chain_network_id WHERE cn.chain_id_prefix = 'uni' AND b.height BETWEEN ${startBlock} AND ${endBlock} AND c.address = '${managerContractAddr}')`)
